...
export async function POST(req:NextRequest){
 ...
 try{
  ...
  const {error:checkError}=await ctx.db.from('participant_payout_methods').select('id').eq('participant_id',ctx.participant.id).eq('status','VERIFIED').eq('is_default',true).maybeSingle();
  if(checkError)return NextResponse.json({error:'Unable to inspect current payout method.'},{status:500});
  const {data,error}=await ctx.db.from('participant_payout_methods').insert({
   participant_id:ctx.participant.id,provider,provider_recipient_reference:encryptSecret(reference),label:label||provider,status:'PENDING',is_default:false
  ...
  return NextResponse.json({ok:true,method:data,message:'Payout method saved and is pending verification.'});
