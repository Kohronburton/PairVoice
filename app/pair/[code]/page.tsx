 ...
 return <main className="flowPage"><nav><a className="logo logoLink" href="/">PAIR<span>VOICE</span></a><button className="language" onClick={()=>setLang(es?'en':'es')}>{es?'EN':'ES'}</button></nav>
  <section className="flowShell narrow">
   ...
   <div className="card">
    ...
    {!invite?.available?<div><h3>{es?'Esta invitación ya no está disponible.':'This invitation is no longer available.'}</h3><p>{es?'Si ya te registraste con este enlace, entra en tu cuenta. Si no, pide a tu compañero/a el enlace más reciente.':'If you already joined with this link, sign in to your account. Otherwise ask your partner for their latest PairVoice link.'}</p><a className="darkCta" href={`/login?lang=${lang}`}>{es?'Abrir mi cuenta →':'Open my account →'}</a>{error&&<p className="error">{error}</p>}</div>:
