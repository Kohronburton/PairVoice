...
do $$
declare
 c uuid; v uuid; referrer uuid; referred uuid; partner uuid; er uuid; ep uuid; v_pair uuid; program uuid;
begin
 ...
 insert into pairs(campaign_id,campaign_version_id,state) values(c,v,'CLIENT_QA') returning id into v_pair;
 insert into pair_members(pair_id,enrollment_id,role,share_basis_points) values(v_pair,er,'A',5000),(v_pair,ep,'B',5000);

 perform approve_pair_and_create_earnings(v_pair,'approve:referral-test','TEST');

 if (select count(*) from referral_commissions where pair_id=v_pair)<>1 then raise exception 'referral_commission_not_created'; end if;
 ...
 if not exists(select 1 from business_funnel_events where pair_id=v_pair and participant_id=referrer and event_name='referral_pair_completed') then raise exception 'referral_telemetry_missing'; end if;

 perform process_referrals_for_approved_pair(v_pair);
 if (select count(*) from referral_commissions where pair_id=v_pair)<>1 then raise exception 'referral_commission_not_idempotent'; end if;
