...
describe('partner invite flow helpers',()=>{
 it('builds a stable invite URL without a double slash',()=>expect(buildPartnerInviteUrl('https://pairvoice.com/',' ab12cd34ef56 ')).toBe('https://pairvoice.com/invite/AB12CD34EF56'));
 it('uses the selected language for share copy',()=>{
  expect(buildPartnerInviteMessage('en','https://pairvoice.com/invite/ABC')).toContain('Join me');
  expect(buildPartnerInviteMessage('es','https://pairvoice.com/invite/ABC')).toContain('Únete conmigo');
 });
